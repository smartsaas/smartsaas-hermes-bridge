#!/usr/bin/env bash
# Usage: trigger-agentic-flow.sh <dataId> <flowId> [inputJson]
# Manually trigger an agentic flow. inputJson: optional input data.
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/agentic-flow/${DATA_ID}/${FLOW_ID}/trigger" \
  -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
  -d "$INPUT_JSON")
HTTP=$(echo "$RES" | tail -n1)
BODY_RES=$(echo "$RES" | sed '$d')
if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ]; then
  echo "Agentic flow triggered."
  echo "$BODY_RES"
else
