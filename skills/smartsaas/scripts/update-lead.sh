#!/usr/bin/env bash
# Usage: update-lead.sh <leadJson>
# Update a CRM lead. leadJson must include _id: {"_id":"...", "name":"...", "status":"..."}
...
LEAD_JSON="$1"
if [ -z "$LEAD_JSON" ]; then
  echo "Error: leadJson (first argument) is required. Include _id field."
  exit 1
fi
CURL_OPTS="-s"; case "$BASE" in *localhost*|*127.0.0.1*) CURL_OPTS="-sk" ;; esac
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/sales/update-lead" \
