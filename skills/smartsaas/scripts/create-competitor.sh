#!/usr/bin/env bash
# Usage: create-competitor.sh <competitorJson>
# Creates a competitor. competitorJson: {"name":"...", "website":"...", "description":"..."}
...
COMPETITOR_JSON="$1"
if [ -z "$COMPETITOR_JSON" ]; then
  echo "Error: competitorJson (first argument) is required."
  exit 1
fi
CURL_OPTS="-s"; case "$BASE" in *localhost*|*127.0.0.1*) CURL_OPTS="-sk" ;; esac
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/competitors" \
  -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
  -d "$COMPETITOR_JSON")
HTTP=$(echo "$RES" | tail -n1)
BODY_RES=$(echo "$RES" | sed '$d')
if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ]; then
  echo "Competitor created."
  echo "$BODY_RES"
else
