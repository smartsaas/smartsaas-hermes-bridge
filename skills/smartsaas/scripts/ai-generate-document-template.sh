#!/usr/bin/env bash
# Usage: ai-generate-document-template.sh <promptJson>
# Generate a document template with {{{placeholders}}}. promptJson: {"description":"...", "documentType":"general", "tone":"professional"}
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/ai/document-template/generate" \
  -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
  -d "$PROMPT_JSON")
HTTP=$(echo "$RES" | tail -n1)
BODY_RES=$(echo "$RES" | sed '$d')
if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ]; then
  echo "Document template generated."
  echo "$BODY_RES"
else
