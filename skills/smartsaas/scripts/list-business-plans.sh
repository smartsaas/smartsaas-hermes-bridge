#!/usr/bin/env bash
# Usage: list-business-plans.sh
# List business plans for the user. Permission: business_plans:read
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X GET "${BASE}/api/protected/business-plans" -H "Authorization: Bearer $KEY")
