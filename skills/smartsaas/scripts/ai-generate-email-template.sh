#!/usr/bin/env bash
# Usage: ai-generate-email-template.sh <promptJson>
# Generate an EmailBuilder email template. promptJson: {"description":"...", "templateType":"general", "tone":"professional"}
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/ai/email-template/generate" \
