#!/usr/bin/env bash
# Usage: ai-analyse-customer-health.sh <dataId>
# Trigger AI customer health analysis on a dataset/CRM folder.
...
DATA_ID="$1"
if [ -z "$DATA_ID" ]; then
  echo "Error: dataId (first argument) is required."
  exit 1
fi
CURL_OPTS="-s"; case "$BASE" in *localhost*|*127.0.0.1*) CURL_OPTS="-sk" ;; esac
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/ai/analyze-customer-health/${DATA_ID}" \
  -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
  -d "{}")
HTTP=$(echo "$RES" | tail -n1)
BODY_RES=$(echo "$RES" | sed '$d')
if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ]; then
  echo "Customer health analysis complete."
  echo "$BODY_RES"
else
