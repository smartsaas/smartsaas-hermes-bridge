#!/usr/bin/env bash
# Usage: ai-quick-social-content.sh <promptJson>
# Generate social post content. promptJson: {"text":"...", "call":"optional CTA"}
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/ai/quick-social-content" \
  -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
  -d "$PROMPT_JSON")
HTTP=$(echo "$RES" | tail -n1)
BODY_RES=$(echo "$RES" | sed '$d')
if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ]; then
  echo "Social content generated."
  echo "$BODY_RES"
else
