#!/usr/bin/env bash
# Usage: update-calendar-event.sh <entryId> <title> <startDate> <endDate> [calendarId] [description] [location]
# Updates an existing calendar event
# Required: entryId, title, startDate, endDate
# Optional: calendarId, description, location, allDay
...
if [ -z "$ENTRY_ID" ] || [ -z "$TITLE" ] || [ -z "$START_DATE" ] || [ -z "$END_DATE" ]; then
  ...
  echo "Usage: update-calendar-event.sh <entryId> <title> <startDate> <endDate> [calendarId] [description] [location]"
  exit 1
fi
