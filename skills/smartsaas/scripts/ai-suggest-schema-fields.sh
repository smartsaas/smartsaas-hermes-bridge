#!/usr/bin/env bash
# Usage: ai-suggest-schema-fields.sh <promptJson>
# Use AI to suggest schema fields for a dataset. 
# promptJson: {"datasetName":"...", "description":"...", "useCase":"..."}
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/ai/data-schema/suggest-fields" \
  -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
  -d "$PROMPT_JSON")
HTTP=$(echo "$RES" | tail -n1)
BODY_RES=$(echo "$RES" | sed '$d')
if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ]; then
  echo "Schema fields suggested."
  echo "$BODY_RES"
else
