#!/usr/bin/env bash
# Usage: get-agent-rules.sh <agentId>
# Get rules/instructions for a specific agent.
...
AGENT_ID="$1"
if [ -z "$AGENT_ID" ]; then
  echo "Error: agentId (first argument) is required."
  exit 1
fi
CURL_OPTS="-s"; case "$BASE" in *localhost*|*127.0.0.1*) CURL_OPTS="-sk" ;; esac
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X GET "${BASE}/api/agents/${AGENT_ID}/rules" \
