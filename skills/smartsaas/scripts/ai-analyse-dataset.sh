#!/usr/bin/env bash
# Usage: ai-analyse-dataset.sh <datasetId>
# Trigger AI analysis on a dataset. Returns insights and patterns.
...
if [ -z "$DATASET_ID" ]; then
  echo "Error: datasetId (first argument) is required."
  exit 1
fi
CURL_OPTS="-s"; case "$BASE" in *localhost*|*127.0.0.1*) CURL_OPTS="-sk" ;; esac
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X GET "${BASE}/api/ai/analyse-dataset/${DATASET_ID}" \
