#!/usr/bin/env bash
# Usage: list-document-templates.sh
# Lists document templates for the signed-in user/company.
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X GET "${BASE}/api/protected/templates/documents" -H "Authorization: Bearer $KEY")
