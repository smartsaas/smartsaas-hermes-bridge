...
#   resource: dataset | data-item | project | work-package | task | campaign | email-template | webpage-template | business-plan
...
if [ ! -f "$SCHEMA_PATH" ]; then
  echo "Error: Unknown resource '$RESOURCE'. Available: dataset, data-item, project, work-package, task, campaign, email-template, webpage-template, business-plan"
  exit 1
fi
cat "$SCHEMA_PATH"
