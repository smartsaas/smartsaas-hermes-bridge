#!/usr/bin/env bash
# Usage: create-agent.sh <agentJson>
# Create a SmartSaaS AI agent. agentJson: {"name":"...", "role":"...", "instructions":"...", "model":"..."}
...
if [ -z "$AGENT_JSON" ]; then
  echo "Error: agentJson (first argument) is required."
  exit 1
fi
CURL_OPTS="-s"; case "$BASE" in *localhost*|*127.0.0.1*) CURL_OPTS="-sk" ;; esac
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/agents/create" \
