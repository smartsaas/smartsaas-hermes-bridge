#!/usr/bin/env bash
# Usage: ai-generate-webpage-template.sh <promptJson>
# Generate a GrapesJS webpage template with CRM widgets. promptJson: {"description":"...", "companyContext":{}}
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/ai/webpage-template/generate" \
  -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
  -d "$PROMPT_JSON")
HTTP=$(echo "$RES" | tail -n1)
BODY_RES=$(echo "$RES" | sed '$d')
if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ]; then
  echo "Webpage template generated."
  echo "$BODY_RES"
else
