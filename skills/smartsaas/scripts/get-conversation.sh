#!/usr/bin/env bash
# Usage: get-conversation.sh <conversationId>
...
if [ -z "$CONVERSATION_ID" ]; then
  echo "Error: conversationId (first argument) is required."
  exit 1
fi
CURL_OPTS="-s"; case "$BASE" in *localhost*|*127.0.0.1*) CURL_OPTS="-sk" ;; esac
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X GET "${BASE}/api/conversation/messages/${CONVERSATION_ID}" \
