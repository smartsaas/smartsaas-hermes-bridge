#!/usr/bin/env bash
# Usage: ai-generate-knowledge.sh <promptJson>
# Use AI to generate a knowledge article. promptJson: {"topic":"...", "context":"...", "format":"article|summary"}
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/ai/generate-knowledge" \
