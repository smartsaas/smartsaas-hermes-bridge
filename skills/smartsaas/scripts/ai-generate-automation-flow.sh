#!/usr/bin/env bash
# Usage: ai-generate-automation-flow.sh <promptJson>
# Use AI to generate an automation workflow from a natural language description.
# promptJson: {"description":"When a new lead is created, send a welcome email and create a task", "context":"..."}
...
RES=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE}/api/ai/automation-flow/generate" \
  -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
  -d "$PROMPT_JSON")
HTTP=$(echo "$RES" | tail -n1)
BODY_RES=$(echo "$RES" | sed '$d')
if [ "$HTTP" -ge 200 ] && [ "$HTTP" -lt 300 ]; then
  echo "Automation flow generated."
  echo "$BODY_RES"
else
